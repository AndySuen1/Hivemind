...
- ⚠️ `BotInstance` 构造时**快照** bot 配置；PATCH 改了 systemPrompt/temperature/tools/providerId/allowedRequesters/discordToken/enabled 任一，`api.ts` 必须**重启实例**才生效（`allowedRequesters` 是访问控制，漏重启=安全空窗）。
